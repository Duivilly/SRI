		...
		for j in range(img.shape[1]):
			if(j+2 < img.shape[1] and i+2 < img.shape[0]):
				...
				if(abs(pixel-v1) > limiar and abs(pixel-v2) > limiar and abs(pixel-v3) > limiar and abs(pixel-v4) > limiar):
					histogramBorder[pixel]= histogramBorder[pixel] + 1
				else:
					histogramInside[pixel]= histogramInside[pixel] + 1
...
for i in range(8942):
	...
	id_url_rn= id_url.split('jpg')[0]+'jpg'
	text= str(line_data[2]).replace('\n','')[1:]
	# verifique se ja foi baixado
#	if os.system('ls | grep '+id_url_rn) != 0:
	if True:
		# obter imagem da web
		#os.system('wget '+url)
		...
		if True:
			# rename
			#os.system('mv '+id_url+' '+id_url_rn)
			try:
				# insert into db
				db.execute("""INSERT INTO pages_baseimage(id, search_image, search_text, descriptorBIC, descriptorBIC_part) VALUES (?,?,?,?,?);""", (count, 'images/'+id_url_rn, text, str(extractImageBIC(id_url_rn)), str(extractImageBIC_part(id_url_rn))))
				# update into db
