...
def extractImageBIC(pathImage):
	...
	contBorder= 0
	contInside= 0
	limiar= 12 #limite da diferenca entre borda/interior
...
def compareHistogramBIC(histogram1, histogram2, limiar):
    ...
    for i in range(len(histogram1)):
        md= abs(int(histogram1[i])-int(histogram2[i])) +md
    return [limiar, md]
			...
			if search_text == '' and search_image == None:
				...
				if search_image != None:
					...
					for q_img in allBaseImages:
						...
						resultcompareHistogramBIC= compareHistogramBIC(extractImageUpload, normalizeListBic(q_img.descriptorBIC), True)
